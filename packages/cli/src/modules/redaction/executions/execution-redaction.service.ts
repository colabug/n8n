...
export class ExecutionRedactionService implements ExecutionRedaction {
	...
	async processExecutions(
		...
		for (let i = 0; i < executions.length; i++) {
			...
			const context: RedactionContext = {
				...
				userCanReveal,
				hasDynamicCredentials: enforceDynCredRedaction,
				memo: new Map(),
			};
	 ...
	 *
	 * Returns false when the execution has no `executedByUserId` (e.g. rows
	 * created before this field was tracked, or system-triggered runs with no
	 * attributable user) — falling back to the redacted-for-everyone default.
	 */
	private isOwnDynamicCredentialsExecution(
		execution: RedactableExecution,
		userId: string,
	): boolean {
		// A null/undefined `executedByUserId` will never strict-equal the
		// requester's string id, so a single comparison covers both the
		// "no recorded user" and "different user" cases.
		return execution.executedByUserId === userId;
	}
