...
type QueuedCreateResult =
	| { type: 'resolve'; value: MockSandbox }
...
const queuedCreateResults: QueuedCreateResult[] = [];
...
function makeDaytonaClientForLog(config: unknown): DaytonaClientLog {
	...
	const get = jest.fn<Promise<MockSandbox>, [string]>().mockImplementation(async (_name: string) => {
	...
	const create = jest
		...
		.mockImplementation(async () => {
			...
			if (queued?.type === 'reject') return await Promise.reject(queued.error);
			if (queued?.type === 'resolve') return await Promise.resolve(queued.value);
...
describe('DaytonaSandbox (create strategies)', () => {
	it('falls back from snapshot creation to image creation and preserves sandbox labels', async () => {
		...
		queuedGetErrors.push(new DaytonaNotFoundError('not found'));
		queuedCreateResults.push(
			{ type: 'reject', error: snapshotError },
			{ type: 'resolve', value: makeMockSandbox('remote-sandbox') },
		);
	...
	it('reports image strategy failures and rethrows', async () => {
		...
		queuedGetErrors.push(new DaytonaNotFoundError('not found'));
		queuedCreateResults.push({ type: 'reject', error: imageError });
